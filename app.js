...
function riskScore(p) {
  const liq=Number(p?.liquidity?.usd||0), age=p?.pairCreatedAt?(Date.now()-Number(p.pairCreatedAt))/3600000:9999;
  const tx=(Number(p?.txns?.h1?.buys||0)+Number(p?.txns?.h1?.sells||0));
  const v=Number(p?.volume?.h24||0);
  let r=35;
  if(liq<25000)r+=35; else if(liq<50000)r+=25; else if(liq<100000)r+=15; else if(liq<250000)r+=7;
  if(age<1)r+=12; else if(age<6)r+=6;
  if(tx<20)r+=8;
  if(v>0 && liq>0 && v/liq>25)r+=8;
  return Math.max(0,Math.min(100,Math.round(r)));
}
function riskLabel(r){return r>=70?'HIGH':r>=45?'MEDIUM':'LOW';}
function socialScoreForToken(t){
  ...
  return Math.max(0,...socialPosts.map(p=>{
    const hits=(p.tokenHits||[]).some(x=>x.toLowerCase()===('
  const liq = Number(p?.liquidity?.usd || 0);
...
function tokenRow(t, i) {
  return `<tr><td>${i + 1}</td><td><div class="token-cell"><span class="token-avatar ${t.color}">${t.letter}</span><span><b>${t.symbol}</b><small>${t.name}</small></span></div></td><td>${fmtUsd(t.price)}</td><td class="${trendClass(t.h1)}">${t.h1 >= 0 ? '+' : ''}${t.h1.toFixed(2)}%</td><td class="${trendClass(t.h24)}">${t.h24 >= 0 ? '+' : ''}${t.h24.toFixed(2)}%</td><td>${fmtCompact(t.volume)}</td><td>${fmtCompact(t.liquidity)}</td><td><span class="signal ${t.signal.toLowerCase()}">${t.signal} ${t.combined}</span><small class="risk-mini">${riskLabel(t.risk)} RISK</small></span></td><td><button class="star" aria-label="Open ${t.symbol}" onclick="window.open('${t.url}','_blank')">↗</button></td></tr>`;
}
function renderFomoLeaderboard(items){const el=document.querySelector('#fomo-leaderboard');if(!el)return;if(!items.length){el.innerHTML='<span class="leader-loading">Loading live FOMO rankings…</span>';return;}const top=items.slice(0,10);const cards=top.map((t,i)=>`<a class="leader-item" href="${t.url}" target="_blank" rel="noreferrer"><span class="leader-rank">#${i+1}</span><span class="leader-symbol">${t.symbol}</span><span class="leader-score">${t.combined} RADAR</span><span class="leader-move ${trendClass(t.m5)}">${t.m5>=0?'+':''}${t.m5.toFixed(1)}%</span></a>`).join('');el.innerHTML=cards+cards;}
...
function renderFilteredTokens(){const q=searchValue.trim();let list=tokens;if(activeFilter!=='all')list=list.filter(t=>activeFilter==='trending'?t.fomo>=65:activeFilter==='gainers'?t.h24>0:activeFilter==='new'?t.ageHours<24:true);if(q)list=list.filter(t=>`${t.symbol} ${t.name}`.toLowerCase().includes(q));renderTokens(list);}
...
function renderEarly(items) {
  ...
  earlyOpportunities.innerHTML = items.slice(0, 3).map(t => {
    ...
    return `<article class="early-card"><div class="early-head"><div class="token-cell"><span class="token-avatar ${t.color}">${t.letter}</span><span><b>${t.symbol}</b><small>${t.name}</small></span></div><span class="signal ${t.signal.toLowerCase()}">${t.signal}</span></div><div class="early-score"><strong>${t.combined}</strong><span>/100 RADAR</span></div><div class="score-bar"><span style="width:${t.fomo}%"></span></div><div class="early-metrics"><span><small>AGE</small><b>${age}</b></span><span><small>5M</small><b class="${trendClass(t.m5)}">${t.m5 >= 0 ? '+' : ''}${t.m5.toFixed(1)}%</b></span><span><small>1H</small><b class="${trendClass(t.h1)}">${t.h1 >= 0 ? '+' : ''}${t.h1.toFixed(1)}%</b></span><span><small>BUYS</small><b>${buyRatio}%</b></span></div><button class="watch-button" onclick="window.open('${t.url}','_blank')">Open on Axiom <span>↗</span></button></article>`;
  ...
  try {
    ...
    const fresh = results.filter(r => r.status === 'fulfilled' && r.value.pair).map(r => {
      ...
      return {
        ...
        fomo: score, social: 0, combined: score, risk: riskScore(p), signal: signalFor(score, p), url: p.url, buys, sells, ageHours
      };
    }).sort((a, b) => b.fomo - a.fomo);
    if (!fresh.length) throw new Error('No live Solana pairs returned');
    fresh.forEach(t=>{t.social=socialScoreForToken(t);t.combined=combinedScore(t);});
...
async function fetchSocialRadar(){
  ...
  try{
    ...
    if(tokens.length){tokens.forEach(t=>{t.social=socialScoreForToken(t);t.combined=combinedScore(t);});tokens.sort((a,b)=>b.combined-a.combined);renderTokens(tokens);renderEarly(tokens);renderLeader(tokens);renderFomoLeaderboard(tokens);renderStats(tokens);}
...
+t.symbol).toLowerCase()) || p.text.toLowerCase().includes(hay);
    return hits ? p.score : 0;
  }));
}
function combinedScore(t){
  const social=t.social||0;
  return Math.max(0,Math.min(100,Math.round(t.fomo*0.72+social*0.28)));
}
function detectBreakouts(items){
  ...
  items.forEach(t=>{
    ...
    if(prev){
      const move=t.m5-prev.m5;
      const volumeJump=prev.volume>0?t.volume/prev.volume:0;
      if(move>=6 || volumeJump>=2.5 || (t.social>=75 && prev.social<75)) events.push({symbol:t.symbol,move:move.toFixed(1),social:t.social,score:t.combined});
    }
  });
  if(events.length) breakingEvents=events.slice(0,5).map(e=>({...e,at:now}));
  previousTokens=new Map(items.map(t=>[t.symbol,{m5:t.m5,volume:t.volume,social:t.social}]));
}
function renderBreaking(){
  const el=document.querySelector('#breaking-feed'); if(!el)return;
  const fresh=breakingEvents.filter(e=>Date.now()-e.at<90000);
  breakingEvents=fresh;
  el.innerHTML=fresh.map(e=>`<article class="breaking-item"><span class="breaking-icon">🚨</span><div><b>${e.symbol}</b> <span>BREAKOUT DETECTED</span><small>Momentum +${e.move}% • Social ${e.social} • Radar ${e.score}</small></div></article>`).join('') || '<div class="breaking-empty">Watching for sudden moves, volume spikes, and social acceleration…</div>';
}
